...
public abstract class AbstractHttpAction extends AbstractAction {
	...
	public AbstractHttpAction() {
		...
		addOption(Constants.NEXUS_USERNAME, true,
				"Username for Nexus Authentication");
	...
	private void prepareCommonInputs() throws AutomicException {
		...
		try {
			this.connectionTimeOut = CommonUtil.parseStringValue(
					getOptionValue(Constants.CONNECTION_TIMEOUT),
					Constants.MINUS_ONE);
			NexusValidator.lessThan(connectionTimeOut, Constants.ZERO,
					"Connect Timeout");
			this.readTimeOut = CommonUtil
					.parseStringValue(getOptionValue(Constants.READ_TIMEOUT),
							Constants.MINUS_ONE);
			NexusValidator
					.lessThan(readTimeOut, Constants.ZERO, "Read Timeout");
			...
			String msg = String.format(
					ExceptionConstants.INVALID_INPUT_PARAMETER, "URL", temp);
	...
	protected WebResource getClient() throws AutomicException {
		if (client == null) {
			client = HttpClientConfig.getClient(baseUrl.getScheme(),
